...
These are combined into a new HTML document, which is beautified using a Bootswatch stylesheet and then loaded in the `WebView`.


==== Design Considerations

===== Aspect: When reader view is generated

* **Alternative 1 (current choice):** Generate reader view from the currently loaded web page.
** Pros: More flexible in the situations it can be used, will use less disk space.
** Cons: May have slight performance issues because it needs to be generated on-the-fly, more difficult to implement.
