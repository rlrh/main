...
`EntryBookParser#parseCommand(String)` is the fallback operation used when it's children
...
If both `Title` and `Description` are already specified by the user, `AddCommand` just adds the `Entry` to the `Model`.
Otherwise, `AddCommand` attempts to find replacement candidates for a missing `Title` or `Description` heuristically, then updates the `Entry` with the best replacement and adds it to the `Model`.
...
and the last attempt is by using the Crux library to process the Document object and extract metadata.
...
The reader view gives users a more focused and comfortable reading experience by removing clutter and formatting content into a clean and easy-to-read layout.

Using the `view` command, users can switch between the `browser` and `reader` view modes.

==== Current Implementation

The two available view modes are specified in the `ViewMode` enum.
...
If an original web page is loaded but the `reader` view mode is selected, the reader view is generated and loaded,
except when the currently loaded web page is the default blank page or error page.

The Crux and Jsoup external libraries do the heavy lifting of parsing the raw HTML currently loaded in the `WebView` to extract useful metadata and content.
...
The feed subscription system requires `ModelManager` to contain more
than one `EntryBook`. Since we should only show a single `EntryBook` at
a time, we have to decouple the `EntryBook` that is displayed from the
ones that are in `ModelManager`.

===== Decoupling displayed `EntryBook`

Decoupling the displayed `EntryBook` means that the following additional
methods are implemented

* `Model#displayEntryBook(EntryBook)` — shows the `EntryBook` provided
in the user interface

Given below is an example usage scenario and how this mechanism behaves.

1.  The user launches the application. The `ModelManager` will default
to displaying the `EntryBook` which corresponds to the default (list)
context.
2.  The user executes the `feed [valid_feed_url]` command to view a web
feed. The `feed` command fetches and parses the web feed into an
`EntryBook` and calls `Model#displayEntryBook` with it.
3.  In the case of the `feed` command, the context changes to `search`
(internal working name). In the `search` context the `EntryBook` is
read-only, and you can import links from the currently displayed
`EntryBook` into the `list` context `EntryBook`.

===== Multiple `EntryBook`’s

Some contexts (such as `feed` and `archive` context) requires
maintaining a separate `EntryBook` each. Since we will only need at most
4 separate `EntryBook`’s, `Model` will basically contain differently
named `EntryBook`’s with their own accessors.
...
==== Singleton pattern (chosen pattern)
...
* Allows for convenient access to the `Network`
...
==== TODO: Some other pattern we considered but we forgot

TODO: Add details here

==== TODO: Comparison of methods and why we went with Singleton
