...
(refer to the <<UserGuide.adoc#context-switching-commands, User Guide>> for the list of these commands).
...
Given below is an example usage scenario of how the contexts and parsers restrict available commands.

1.  The user launches the application.
2.  The `ModelManager` will default to `ModelContext.CONTEXT_LIST` context.
3.  The user executes the `archive` command with correct arguments.
4.  `EntryBookListParser` parses the command successfully. Command executes
5.  The user erroneously executes the `unarchive` command.
6.  `EntryBookListParser` tries to parse it and fails. The application reports it as an "Unknown command".
7.  The user executes the `archives` command to view his archives.
8.  In the execution of this `ArchivesCommand`, `model.setContext(ModelContext)` is called to change the
context to `ModelContext.CONTEXT_ARCHIVES`.
9.  The user executes the `unarchive` command with correct arguments.
10.  `EntryBookArchivesParser` parses the command successfully. Command executes
